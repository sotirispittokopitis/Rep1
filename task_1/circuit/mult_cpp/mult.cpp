...
void BalanceCheck_3_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void BalanceCheck_3_run(uint ctx_index,Circom_CalcWit* ctx);
...
uint get_total_signal_no() {return 204;}

uint get_number_of_components() {return 4;}

uint get_size_of_input_hashmap() {return 256;}

uint get_size_of_witness() {return 195;}
...
// load src
Fr_shr(&expaux[1],&signalValues[mySignalStart + 193],&lvar[3]); // line circom 32
...
Fr_eq(&expaux[0],&lvar[1],&signalValues[mySignalStart + 193]); // line circom 38
...
// load src
Fr_sub(&expaux[0],&circuitConstants[2],&ctx->signalValues[ctx->componentMemory[mySubcomponents[0]].signalStart + 192]); // line circom 98
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
for (uint i = 0; i < 1; i++){
...
ctx->componentMemory[coffset].templateName = "BalanceCheck";
ctx->componentMemory[coffset].signalStart = soffset;
ctx->componentMemory[coffset].inputCounter = 2;
ctx->componentMemory[coffset].componentName = componentName;
ctx->componentMemory[coffset].idFather = componentFather;
...
BalanceCheck_3_create(1,0,ctx,"main",0);
BalanceCheck_3_run(0,ctx);
}
